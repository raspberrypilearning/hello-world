...
# Funktionsdefinitionen
def wuerfel_werfen():
  wurf = randint(1, 6) # Erzeuge eine Zufallszahl zwischen 1 und 6 und speichere sie in der Variable 'wurf'
  print('Das hast Du gewürfelt:', wurf, feuer * wurf) # Wiederhole das Feuer-Emoji so oft wie es der gewürfelten Zahl entspricht
...
def würfel_werfen():   
    ...
    print('Du hast gewürfelt:', wurf, feuer * wurf)

--- /code ---

**Tipp:** Wenn Du ein Apostroph `'` oder Anführungszeichen `"` ausgeben möchtest, musst Du davor einen Backslash `\` einfügen, sodass Python weiß, dass es Teil des Textes ist.
