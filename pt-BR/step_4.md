...
Em um teclado, os dois pontos `:` estão na mesma tecla que o ponto e vírgula, ao lado da tecla <kbd>L</kbd>: segure <kbd>Shift</kbd> e toque <kbd>;</kbd> para digitar um `:`. O sublinhado `_` está na mesma tecla que o `-`, próximo ao <kbd>0</kbd>, segure <kbd>Shift</kbd> e toque <kbd>-</kbd> para digitar um `_`.
...
![O editor de código que mostra a linha de código dentro da função <code>rolar_dado</code> não foi recuado. A linha de código com o erro é destacada. O código foi executado, com o erro 'SyntaxError: bad input on line 17 in main.py'.](images/indent_error.png)
...
Sua função pode usar a variável emoji 🔥. O código `print(fogo * 3)` gera três emojis de fogo '🔥🔥🔥'. Você precisa gerar o número correto de emojis para corresponder ao número rolado.
...
# Definições de função
def rolar_dado(): rolar = randint(1, 6) # Gere um número aleatório entre 1 e 6 e armazene-o na variável 'rolar' print('Você rolou um', rolar, fogo * rolar) # Repita o disparar emoji para corresponder ao rolo de dados aleatório
...
rolar= randint(1, int(maximo)) # Use max para determinar o número de lados que o dado tem print('Você rolou um', rolar, fogo * rolar)
